...
class PlanningGraph():
	...
	def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
		...
		for effnode in node_a1.effnodes:
			opp_literal = expr('~{}'.format(effnode.symbol))
			if not effnode.is_pos:
				opp_literal = expr(effnode.symbol)
			for effect_a2 in node_a2.effnodes:
				if opp_literal == effect_a2.literal:
					result = True
		return result
	...
	def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
		...
		#for each precondition in node 1
		for prenode_a1 in node_a1.prenodes:
			#get its negative literal
			opp_literal = expr('~{}'.format(prenode_a1.symbol))
			if not prenode_a1.is_pos:
				opp_literal = expr(prenode_a1.symbol)
			#compare it with each effect in node 2
			for effnode_a2 in node_a2.effnodes:
				#and see if the negative literal matches literal of effect
				if opp_literal == effnode_a2.literal:
					result = True

		#repeat the same in opposite
		for prenode_a2 in node_a2.prenodes:
			opp_literal = expr('~{}'.format(prenode_a2.symbol))
			if not prenode_a2.is_pos:
				opp_literal = expr(prenode_a2.symbol)
			for effnode_a1 in node_a1.effnodes:
				if opp_literal == effnode_a1.literal:
					result = True
	...
	def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
		...
		result = False
		opp_literal = expr('~{}'.format(node_s1.symbol))
		if not node_s1.is_pos:
			opp_literal = expr(node_s1.symbol)
		if opp_literal == node_s2.literal:
			result = True
		return result
	...
	def h_levelsum(self) -> int:
		...
		for each_goal in self.problem.goal:
			...
			for s_level in range (len(self.s_levels)):
				for s_node in self.s_levels[s_level]:
					if s_node.literal == each_goal:
						level_sum += s_level
						first = False
						break
				if not first:
